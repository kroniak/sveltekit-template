<script lang="ts">
  ...
  import { authClient, oAuth2Providers, signIn } from "$lib/auth-client";
...
{#snippet socialButton(provider: OAuth2Providers)}
  <div class="relative">
    <Button
      variant="outline"
      type="button"
      class="w-full"
      onclick={async () => await signIn(provider.id, redirectUrl)}
    >
      <provider.icon />
      Login with <span class="capitalize">{provider.name}</span>
    </Button>
